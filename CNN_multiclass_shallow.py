...
os.makedirs('SIIM-ISIC/Descriptives', exist_ok = True)

training_dist = training_functions.category_distribution(TRAIN_PATH)
training_dist.to_csv('SIIM-ISIC/Descriptives/Training_dist.csv')
validation_dist = training_functions.category_distribution(VAL_PATH)
validation_dist.to_csv('SIIM-ISIC/Descriptives/Validation_dist.csv')

mel_num = train_generator.class_indices.get('MEL')
...
categories = [x for x in training_dist['Category']]
counts = [x for x in training_dist['Count']]

weights = {}

for i in range(len(categories)):
    weights[i] = (1/counts[i])*total_num/len(categories)
...
for drop in dropout_grid:
    ...
    # Setting up paths and folders for output
    
    output_str = f'Multiclass-shallow/dropout_{drop}'
    ...
    # Setting up callbacks including Tensorboard
    ...
    # Evaluating on the SIIM-ISIC data
    
    train_df, _ = evaluation_functions.prediction_dataframes(model, False, train_generator, train_generator)
    train_df.to_csv(csv_str+'/train_df.csv', index = False)
    
    test_df, submit_df = evaluation_functions.prediction_dataframes(model, False, train_generator, test_generator)
